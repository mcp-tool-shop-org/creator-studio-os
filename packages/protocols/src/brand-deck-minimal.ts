...
import type { ProjectV2, Scene } from "@creator-studio-os/core";
...

  {
    ...
    if (isResumed(stepName, iHash, resumeManifest)) {
      ...
      if (dryRun) {
        ...
        const { cloneTemplate } = await import("@creator-studio-os/motion");
        const { editText, patchSiblingText } = await import("@creator-studio-os/motion");
        const { renderViaCompressor } = await import("@creator-studio-os/motion");
        const { waitFor } = await import("@creator-studio-os/compressor");
        const cfg = loadConfig();
  ...
  // -------------------------------------------------------------------------
  {
    ...
    if (isResumed(stepName, iHash, resumeManifest)) {
      ...
      if (dryRun) {
        ...
        const { validateFcpxmlAgainstDtd } = await import("@creator-studio-os/fcp");
        const cfg = loadConfig();
  ...
  // -------------------------------------------------------------------------
  {
    ...
    if (isResumed(stepName, iHash, resumeManifest)) {
      ...
      if (dryRun) {
        ...
        try {
          const { openWithApp } = await import("@creator-studio-os/core");
    ...
    if (isResumed(stepName, iHash, resumeManifest)) {
      ...
      if (dryRun) {
        ...
        try {
          ...
          const { encodeJob } = await import("@creator-studio-os/compressor");
