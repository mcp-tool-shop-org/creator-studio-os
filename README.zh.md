...
MCP控制平面，用于Apple Creator Studio应用程序。通过Claude或任何MCP客户端驱动**Final Cut Pro**、**Compressor**、**Motion**、**Pixelmator Pro**、**Logic Pro**、**Keynote**、**Pages**和**Numbers**。通过JSON规范创建视频输出，在无界面模式下渲染Motion的片头，使用Compressor进行编码，并在一个跨应用程序的流水线中生成品牌资产。
...
`creator-studio-os`是桥梁：Claude以JSON规范编写时间线，服务器构建+验证FCPXML，触发FCP导入，通过Compressor在无界面模式下渲染Motion片头模板，并使用Pixelmator Pro生成品牌资产——所有这些都在一个跨应用程序的流水线中完成。

## 安全性

`creator-studio-os`完全在设备上运行。它：

- 启动`osascript`，目标是根据Bundle ID（而不是文件名）来选择应用程序。
- 仅在`CREATOR_STUDIO_DATA_DIR`目录下写入文件，不涉及任何系统文件或FCP库的内部文件。
...
检查平台、`osascript`、`xmllint`、Final Cut Pro安装情况、FCPXML 1.14 DTD、数据目录，并对捆绑的DTD进行FCPXML的完整流程测试。
...
| `fcp_project_create` | 创建项目目录 + `project.json`文件 |
| `fcp_project_info` | 读取项目元数据 + 解析后的路径 |
| `fcp_fcpxml_build` | 从JSON规范构建FCPXML 1.14文件 |
| `fcp_fcpxml_validate` | 验证FCPXML文件是否符合捆绑的DTD |
| `fcp_fcpxml_write` | 将FCPXML文件写入`projects/<name>/fcp/`目录 |
...
| `motion_template_clone` | 克隆 `.motn` 模板到新的路径 |
...
注意：`motion_template_set_param` 和 `motion_render_via_compressor` 在任何 MCP 全局范围内都没有先例——无头 Motion OZML 修改和渲染仅由 csos 实现。
...
| `pixelmator_compose_brand_card` | 合成带有标题文本的色相旋转品牌卡 |
...
### Keynote / Pages / Numbers (总共 18 个工具)
...
| `csos_protocol_run` | 运行跨应用程序的端到端协议（异步，分步流式传输） |
| `csos_protocol_list` | 列出所有已注册的协议 |
| `csos_protocol_describe` | 描述协议的步骤和目的。 |

## 推荐的配置，使用 tool-compass

[tool-compass](https://github.com/mcp-tool-shop-org/tool-compass) 是一个语义化的 HNSW 网关，它能够根据自然语言意图找到合适的工具——这对于管理 78 个工具，服务于 8 个应用程序来说至关重要。

```bash
pip install tool-compass
```

“烟雾测试”框架验证了第七阶段的 12 个典型查询。任何描述更改，如果导致目标不在前 3 名，且得分 > 0.4，则“烟雾测试”失败。
...
| 检查 | 什么 |
|-------|------|
| **A. Security** | [`SECURITY.md`](./SECURITY.md), [`docs/threat-model.md`](./docs/threat-model.md)，不包含任何敏感信息，不收集任何遥测数据，不使用任何网络连接。 |
| **B. Errors** | `CreatorStudioError { code, message, hint }`，CLI 退出码，不包含原始堆栈信息。 |
| **C. Docs** | 此 README 文件，[`CHANGELOG.md`](./CHANGELOG.md)，[`LICENSE`](./LICENSE)，`--help` 参数的描述准确无误。 |
| **D. Hygiene** | `npm test`，`npm run typecheck`，版本号与标签匹配，`npm audit`，打包过程干净。 |
...
- **v1.8.x** — `patchSiblingText` 文本边界验证：当传入文本可能超出固定 Motion 模板渲染边界时，会显示 ledger 警告。
