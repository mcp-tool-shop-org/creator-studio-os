<p align="center">
  <a href="README.ja.md">日本語</a> | <a href="README.zh.md">中文</a> | <a href="README.fr.md">Français</a> | <a href="README.md">English</a>
</p>
...
- Executa `osascript` direcionado a aplicativos por ID de pacote (nunca por nome de arquivo)
- Escreve apenas dentro de `CREATOR_STUDIO_DATA_DIR` — sem arquivos do sistema, sem detalhes internos da biblioteca do FCP
...
Verifica a plataforma, `osascript`, `xmllint`, instalação do Final Cut Pro, DTD FCPXML 1.14, diretório de dados e executa uma operação de ida e volta de FCPXML usando o DTD incluído.
...
| `fcp_project_info` | Lê metadados do projeto + caminhos resolvidos |
| `fcp_fcpxml_build` | Cria FCPXML 1.14 a partir de uma especificação JSON |
...
| `fcp_project_metadata` | Lê a duração da sequência, a taxa de quadros, o formato de código de tempo |
| `fcp_app_open` / `fcp_app_running` / `fcp_app_activate` | Ciclo de vida |
| `fcp_round_trip_diff` | Compara dois arquivos FCPXML, gera uma diferença estruturada |
...
O Compressor não possui dicionário AppleScript — a interface é a linha de comando (CLI) mais os arquivos `.compressorbatch`. A primeira execução por sessão aciona a validação de permissões da App Store (esperado).

| Ferramenta | Propósito |
|------|---------|
| `compressor_app_open` / `compressor_app_running` | Ciclo de vida |
| `compressor_settings_list` | Enumerar as configurações predefinidas `.compressorsetting`. |
| `compressor_locations_list` | Enumerar os arquivos `.compressorlocation`. |
...
| `compressor_monitor_stream` | Transmitir os quadros de progresso da codificação. |
...
| `compressor_batch_build` | Criar um documento XML `.compressorbatch`. |
...
Consulte [`docs/reference/compressor-cli.md`](./docs/reference/compressor-cli.md).
...
| `motion_render_via_compressor` | Renderizar um `.motn` para vídeo de forma headless, usando o Compressor. |

Observação: `motion_template_set_param` e `motion_render_via_compressor` não possuem precedentes em nenhum MCP globalmente — a mutação e renderização OZML do Motion em modo headless são habilitadas exclusivamente pelo csos.
...
| `pixelmator_batch_export_project_images` | Converter em lote `projects/<name>/images/`. |
...
| `pixelmator_effects_apply` / `pixelmator_effects_catalog` | Pipeline de efeitos de ML. |
| `pixelmator_compose_brand_card` | Criar um cartão de marca com rotação de matiz e texto do título. |
| `pixelmator_hdr_export` | Exportar com mapeamento de tom HDR. |
...
O "harness" de teste valida 12 consultas representativas na Fase 7. Qualquer alteração na descrição que faça com que um resultado saia do top-3 com uma pontuação > 0,4 falha no teste.
...
O CI é executado em `ubuntu-latest` (verificação de tipo + compilação + testes unitários + auditoria). Testes de integração contra aplicativos reais são executados via `npm run smoke:ci` – os ambientes macOS não estão intencionalmente incluídos no CI (custo: macOS ≈ 10 vezes o custo do Linux por minuto).
...
- **v1.8.x** — validação de limites de texto `patchSiblingText`: aviso do ledger quando o texto recebido pode cortar os limites fixos do modelo Motion.
