...
Le dictionnaire AppleScript de Final Cut Pro est en **lecture seule** : vous pouvez lister les bibliothèques et lire les métadonnées, mais vous ne pouvez pas créer de séquences via AppleScript. Le chemin d'intégration pris en charge est l'**importation FCPXML** : rédigez un document FCPXML 1.14 valide, transmettez-le à FCP, et FCP crée le projet.
...
| `fcp_fcpxml_build` | Construit un fichier FCPXML 1.14 à partir d'une spécification JSON |
...
| `fcp_fcpxml_build_write_import` | Cycle complet : construction → validation → écriture → importation |
...
| `fcp_project_metadata` | Lit la durée de la séquence, la fréquence d'images, le format de la chronologie |
...
| `fcp_multicam_build` | Crée un clip multicam à partir des spécifications des angles |
| `fcp_caption_build` | Construire une piste de sous-titres à partir d'une transcription. |
| `fcp_compound_clip_build` | Construire un clip composé à partir de spécifications de séquence imbriquées. |
...
| `compressor_monitor_stream` | Afficher les images de progression de l'encodage en streaming. |
...
| `compressor_batch_build` | Construire un document XML `.compressorbatch`. |
...
| `motion_template_clone` | Cloner un modèle `.motn` vers un nouveau chemin. |
...
Note : `motion_template_set_param` et `motion_render_via_compressor` n'ont aucun équivalent connu dans MCP (Media Conversion Pipeline) global. La modification et le rendu OZML de Motion en mode sans interface graphique sont rendus possibles uniquement par csos.
...
| `pixelmator_batch_export_project_images` | Conversion en lot du répertoire `projects/<name>/images/`. |
| `pixelmator_layer_list` / `pixelmator_layer_create` / `pixelmator_layer_delete` | Gestion des calques. |
| `pixelmator_layer_set_text` / `pixelmator_layer_set_fill` / `pixelmator_layer_move` | Édition des calques. |
| `pixelmator_effects_apply` / `pixelmator_effects_catalog` | Pipeline d'effets ML (Machine Learning). |
...
Logic ne possède pas de dictionnaire AppleScript. Interface : cycle de vie et transmission de l'ouverture de fichiers pour les projets `.logicx`.
...
Les trois partagent une structure AppleScript presque identique. Catalogue complet des formats d'exportation : [`docs/reference/iwork-automation.md`](./docs/reference/iwork-automation.md).
...
| `csos_protocol_run` | Exécution d'un protocole inter-applications de bout en bout (asynchrone, étapes en streaming). |
...
[tool-compass](https://github.com/mcp-tool-shop-org/tool-compass) est une passerelle HNSW sémantique qui trouve l'outil approprié en fonction de l'intention exprimée en langage naturel. Ceci est essentiel lorsque 78 outils sont répartis sur 8 applications.

```bash
pip install tool-compass
```

Le test de validation vérifie 12 requêtes représentatives dans la phase 7. Toute modification de la description qui fait disparaître un résultat du top 3 avec un score > 0,4 entraîne un échec du test de validation.
...
| **C. Docs** | Ce fichier README, [`CHANGELOG.md`](./CHANGELOG.md), [`LICENSE`](./LICENSE), l'option `--help` est précise. |
